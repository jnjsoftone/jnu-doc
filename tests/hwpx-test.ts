...
import { readHwpxAsPlainText, readHwpxAsPlainTextFromUrl, readHwpxAsHTML, readHwpxAsHTMLFromUrl, readHwpxBufferAsHTML, readHwpxAsMarkdown, readHwpxAsMarkdownFromUrl } from '../src/hwpx.js';
...
const main = async () => {
  try {
    // const local = await readHwpxAsPlainText(SAMPLE_PATH);
    const localMarkdown = await readHwpxAsMarkdown(SAMPLE_PATH);
    const localHTML = await readHwpxAsHTML(SAMPLE_PATH);
    // logPreview('Local HWPX Output', local);
    console.log('=== Local HTML HWPX Output ===')
    console.log(localHTML);
    console.log('\n=== Local MarkdownHWPX Output ===')
    console.log(localMarkdown);
  } catch (error) {
    console.error('Failed to read local HWPX sample:', error);
  }
